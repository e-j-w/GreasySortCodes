...
#define EEEDopp_only180degcoinc_mca_SMOL_cxx
#include "common.h"
#include "EEEDopp_only180degcoinc_mca_SMOL.h"

using namespace std;

void EEEDopp_only180degcoinc_mca_SMOL::WriteData(const char* outName){
...
void EEEDopp_only180degcoinc_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
            ...
            for(int tigHitIndAB2 = tigHitIndAB+1; tigHitIndAB2 < sortedEvt.header.numTigHits; tigHitIndAB2++){
                for(int tigHitIndAB3 = tigHitIndAB2+1; tigHitIndAB3 < sortedEvt.header.numTigHits; tigHitIndAB3++){
                    ...
                    if((angle >= 170)&&(angle <= 190)){
                        ...
                        int eDopp3 = (int)(getEDoppFusEvapDirect(&sortedEvt.tigHit[tigHitIndAB3],sortedEvt.header.numCsIHits,sortedEvt.csiHit,gates)/keVPerBin);
                        if(eDopp3>=0 && eDopp3<S32K){
                            double theta = getTigVector(sortedEvt.tigHit[tigHitIndAB3].core,sortedEvt.tigHit[tigHitIndAB3].seg).Theta()*180./PI;
                            mcaOut[getTIGRESSRing(theta)+1][eDopp3]++;
                            mcaOut[getTIGRESSSegmentRing(theta)+7][eDopp3]++;
                            mcaOut[0][eDopp3]++;
                        }
                        break;
                    }
...
int main(int argc, char **argv){
  ...
  EEEDopp_only180degcoinc_mca_SMOL *mysort = new EEEDopp_only180degcoinc_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  double eLow, eHigh;
  printf("Starting EEEDopp_only180degcoinc_mca_SMOL\n");
  ...
  if((argc != 5)&&(argc != 6)){
    ...
    cout << "Arguments: EEEDopp_only180degcoinc_mca_SMOL smol_file EGateLow EGateHigh output_file keV_per_bin" << endl;
