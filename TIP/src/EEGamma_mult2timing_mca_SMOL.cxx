...
#define EEGamma_mult2timing_mca_SMOL_cxx
#include "common.h"
#include "EEGamma_mult2timing_mca_SMOL.h"

using namespace std;

void EEGamma_mult2timing_mca_SMOL::WriteData(const char* outName){
...
void EEGamma_mult2timing_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
          ...
          if((sortedEvt.tigHit[tigHitIndAB].energy >= eLow)&&(sortedEvt.tigHit[tigHitIndAB].energy <= eHigh)){
            for(int tigHitIndAB2 = 0; tigHitIndAB2 < sortedEvt.header.numTigHits; tigHitIndAB2++){
              if(tigHitIndAB2 != tigHitIndAB){
                float tDiff = sortedEvt.tigHit[tigHitIndAB2].timeOffsetNs - sortedEvt.tigHit[tigHitIndAB].timeOffsetNs;
...
int main(int argc, char **argv){
  ...
  EEGamma_mult2timing_mca_SMOL *mysort = new EEGamma_mult2timing_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  double eLow, eHigh;
  printf("Starting EEGamma_mult2timing_mca_SMOL\n");
  ...
  if((argc != 5)&&(argc != 6)){
    ...
    cout << "Arguments: EEGamma_mult2timing_mca_SMOL smol_file EGateLow EGateHigh output_file keV_per_bin" << endl;
