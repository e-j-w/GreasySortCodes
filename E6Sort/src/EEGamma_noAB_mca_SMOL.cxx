...
void EEGamma_noAB_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
            ...
            if(noABHitInd2 != noABHitInd){
              ...
              if((tDiff >= hpgehpgeTGate[0])&&(tDiff <= hpgehpgeTGate[1])){
                ...
                if(eGamma>=0 && eGamma<S32K){
                  double theta = getTigVector(sortedEvt.noABHit[noABHitInd2].core,sortedEvt.noABHit[noABHitInd2].seg).Theta()*180./PI;
...
int main(int argc, char **argv){
  ...
  if((argc != 5)&&(argc != 6)){
    ...
    cout << "Arguments: EEGamma_noAB_mca_SMOL smol_file EGateLow EGateHigh output_file keV_per_bin" << endl;
