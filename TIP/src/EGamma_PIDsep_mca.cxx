...
void EGamma_PIDsep_mca::WriteData(const unsigned int numP, const unsigned int numA){
...
void EGamma_PIDsep_mca::SortData(char const *afile, char const *calfile, const unsigned int numP, const unsigned int numA, double keVPerBin){
              ...
              if(passedtimeGate&(1ULL<<(tigHitIndAB+MAXNUMTIPHIT))){
                ...
                if(!add_hit->BGOFired() && add_hit->GetEnergy() > MIN_TIG_EAB){
                  ...
                  if(eGamma>=0 && eGamma<S32K){
                    ...
                    mcaOut[getTIGRESSRing(thetaDeg)+1][eGamma]++;
                    mcaOut[getTIGRESSSegmentRing(thetaDeg)+7][eGamma]++;
                    mcaOut[0][eGamma]++;
                  }
                }
              }
...
int main(int argc, char **argv){
  ...
  if((argc != 5)&&(argc != 6)){
    ...
    cout << "Arguments: EGamma_PIDsep_mca analysis_tree calibration_file numP numA keV_per_bin" << endl;
