...
void SSB_and_Recoils() {

  int z = 55779; // First Run on target
  TH2F *h1[100];
  TH2F *h2[100];
  TH2F *h3[100];
  double rnum;
  ...
  for(int i=0;i<300;i++) {
    ...
    if(gSystem->AccessPathName(Form("HistFiles/Hist_%i_000.root",(z+i)))){
      ...
      if (q == end(rNum)) {
        ...
        h3[j] = (TH2F*)infile->Get("EMMA-S3/icSumVicPlusSi_s3gate");
        rnum = z+i;
        rnumer = 0;
        ...
        SSB2 = h2[j]->Integral(100,400,0,160000)/temp_current; //y axis values need to be divided by 6 to convert from keV to channels, due to changes made by JW to binning of these histograms in SortCode.h, x axis still has 1:1 mapping
        SSB2er = pow(h2[j]->Integral(100,400,0,160000),0.5)/h2[j]->Integral(100,400,0,160000) * SSB2;

        //since this histo has been changed, these are needed to convert the raw numerical values for the integral (3200,3800,1600,2400) into bins for each. DY 14/9/2021
        //double xLow = (3200-h3[j]->GetXaxis()->GetXmin())/h3[j]->GetXaxis()->GetBinWidth(1);
        //double xHigh = (3800-h3[j]->GetXaxis()->GetXmin())/h3[j]->GetXaxis()->GetBinWidth(1);
        //have to project onto the y-axis so that we can use GetXmin() since GetYmin() isn't a thing
        //TH1D *yProj = h3[j]->ProjectionY();
        //double yLow = (1600-yProj->GetXaxis()->GetXmin())/yProj->GetXaxis()->GetBinWidth(1);
        //double yHigh = (2400-yProj->GetXaxis()->GetXmin())/yProj->GetXaxis()->GetBinWidth(1);
        //REC1 = h3[j]->Integral(xLow,xHigh,yLow,yHigh)/temp_current; //y axis values need to be divided by 6 to convert from keV to channels, due to changes made by JW to binning of these histograms in SortCode.h, x axis still has 1:1 mapping
        //REC1er = pow(h3[j]->Integral(xLow,xHigh,yLow,yHigh),0.5)/h3[j]->Integral(xLow,xHigh,yLow,yHigh) * SSB1;
        j++;
        infile->Close();
  ...
  TGraphErrors *rec1g = new TGraphErrors("Monitoring/Recoils.dat","%lg %lg %lg");
