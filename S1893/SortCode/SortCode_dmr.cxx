...
#define Sortcode_dmr_cxx
#include "SortCode_dmr.h"
...
// Change below with actual target thicknesses
  if (strcmp(target, "two") == 0){
    ...
    EBeam = srim_86KrD->GetAdjustedEnergy(EBeam * 1000, 449./2., 0.001);
    ...
    EBeam = srim_86KrD->GetAdjustedEnergy(EBeam * 1000, 369./2., 0.001);
    ...
    EBeam = srim_86KrD->GetAdjustedEnergy(EBeam * 1000, 164./2., 0.001) ;
 ...
 */
  ...
  for (int jentry = 0; jentry < analentries; jentry++)
  {
	...
	for (int i = 0; i < sharc->GetMultiplicity(); i++){
	   ...
	   double eSharc = sharc_hit->GetEnergy();
	   ...
	   sharcE_front->Fill(100*(detSharc)+frontSharc,eSharc); //this will offset them so that detector one is channels 0-23, det 2 is 100-123, etc.  multiply by 100 starts each new detector at a hunded
           ...
           sharcE_back->Fill(100*(detSharc)+backSharc,sharc_hit->GetBack().GetEnergy());
	   ...
	   sharc_excE_theta->Fill(sharc_hit->GetTheta(),excEng);
	  ...
	  //sharc-tigress coincidences
	   if (tigress){
		for (int tigH = 0; tigH < tigress->GetAddbackMultiplicity(); tigH++){
            ...
            if(gate1D(add_hit->GetTime() - sharc_hit->GetTime(), -100, 800)){
	...
	if (bTrifGoodRecon && 0 == jentry%100 && 100 > trifBraggCounter){
		...
		TVector2 simpleBraggVals = fitBasicBragg(&tempBragg, peak, trifBraggE, trifBraggEDet, true); //simpleBraggVals is the array from fitBasicBragg. it is of the form (dedxPeak,range)
		TVector3 complexBraggVals = fitComplicatedBragg(&tempBragg, newbragg, simpleBraggVals, trifBraggE, trifBraggEDet, true); // complexBraggVals is TVector3(dedxPeak,range,dE0)

	}else if (bTrifGoodRecon && 0 == jentry%100){
		TGraph tempBragg = createTrificBraggGraph(trif);
		TVector2 simpleBraggVals = fitBasicBragg(&tempBragg, peak, trifBraggE, trifBraggEDet); //simpleBraggVals is the array from fitBasicBragg. it is of the form (dedxPeak,range)
		TVector3 complexBraggVals = fitComplicatedBragg(&tempBragg, newbragg, simpleBraggVals, trifBraggE, trifBraggEDet); // complexBraggVals is TVector3(dedxPeak,range,dE0)
  ...
  TDirectory *sharcProj = myfile->mkdir("SHARC_projections");
  sharcProj->cd();
  sharcProjList->Write();
  myfile->cd();
