...
#define EEGamma_noAB_noSupp_mult2timing_mca_SMOL_cxx
#include "common.h"
#include "EEGamma_noAB_noSupp_mult2timing_mca_SMOL.h"

using namespace std;

void EEGamma_noAB_noSupp_mult2timing_mca_SMOL::WriteData(const char* outName){
...
void EEGamma_noAB_noSupp_mult2timing_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
  ...
  for(Long64_t jentry = 0; jentry < sentries; jentry++){
    ...
    if(!(sortedEvt.header.metadata & (uint8_t)(1U <<1))){
      //no suppressor was fired
...
int main(int argc, char **argv){
  ...
  EEGamma_noAB_noSupp_mult2timing_mca_SMOL *mysort = new EEGamma_noAB_noSupp_mult2timing_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  double eLow, eHigh;
  printf("Starting EEGamma_noAB_noSupp_mult2timing_mca_SMOL\n");

  if((argc != 5)&&(argc != 6)){
    cout << "Generates TIGRESS mca spectra for PID and time separated data." << endl;
    cout << "Arguments: EEGamma_noAB_noSupp_mult2timing_mca_SMOL smol_file EGateLow EGateHigh output_file keV_per_bin" << endl;
