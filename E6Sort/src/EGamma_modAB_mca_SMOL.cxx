...
uint8_t ABHitMapping[MAX_HITS_PER_EVT]; //arrays specifying which hits correspond to which addback hits
double addbackE[MAX_HITS_PER_EVT];
double addbackT[MAX_HITS_PER_EVT];
...
uint64_t SortData(const char *sfile, const double ABRad, const uint8_t sameCloverOnly, const uint8_t forwardPos, const double keVPerBin, const uint8_t discardPileup, const uint8_t discardAnySuppressor){
    ...
    for(uint8_t i=0;i<(NGRIFPOS*4);i++){ //first core
        for(uint8_t j=0;j<(NGRIFPOS*4);j++){ //coinc core
            if(i!=j){
                ...
                for(uint8_t k=0;k<(NGRIFPOS*4);k++){ //other core next to the first core, which could be addback'd with it
                    if((k!=i)&&(k!=j)){
                        if((!sameCloverOnly) || ((i/4) == (k/4))){
                            if(getGRIFFINHitDistance(i,k,forwardPos) < ABRad){
    ...
    for(Long64_t jentry = 0; jentry < sentries; jentry++){
        ...
        if((discardAnySuppressor == 1)&&(sortedEvt.header.metadata & (uint8_t)(1U << 1))){
            continue; //skip event where suppressor fired
        }
        ...
        for(int noABHitInd = 0; noABHitInd < sortedEvt.header.numNoABHits; noABHitInd++){
            if(noABHitInd < MAX_HITS_PER_EVT){
                ...
                for(int noABHitInd2 = noABHitInd+1; noABHitInd2 < sortedEvt.header.numNoABHits; noABHitInd2++){
                    if(noABHitInd2 < MAX_HITS_PER_EVT){
                        ...
                        if((!sameCloverOnly) || (((sortedEvt.noABHit[noABHitInd].core & 63U)/4) == ((sortedEvt.noABHit[noABHitInd2].core & 63U)/4))){
                            if(getGRIFFINHitDistance(sortedEvt.noABHit[noABHitInd].core & 63U,sortedEvt.noABHit[noABHitInd2].core & 63U,forwardPos) < ABRad){
                                ...
                                if(tDiff <= ADDBACK_TIMING_GATE){ //timing condition
                                    if(!(abHitBuildFlags & (1UL << noABHitInd))){
                                        ...
                                        if(!(abHitBuildFlags & (1UL << noABHitInd2))){
                                            ...
                                            addbackT[numABHitsBuilt] = sortedEvt.noABHit[noABHitInd].timeOffsetNs;
                                            if(!(abHitBuildFlags & (1UL << noABHitInd))){
                ...
                if(abHitBuilt != 0){
                    ...
                    numABHitsBuilt++;
                    if(numABHitsBuilt>=MAX_HITS_PER_EVT){
                        break;
                    }
                }
            }
        ...
        for(uint8_t projABHitInd=0; projABHitInd < numABHitsBuilt; projABHitInd++){
        ...
        for(int noABHitInd = 0; noABHitInd < sortedEvt.header.numNoABHits; noABHitInd++){
            ...
            if((!(usedHits & (1UL << noABHitInd))) && ((!(abHitBuildFlags & (1UL << noABHitInd))) || (!(usedABHits1 & (1UL << ABHitMapping[noABHitInd]))))){
                ...
                if(abHitBuildFlags & (1UL << noABHitInd)){
                    ...
                    eGamma1 = (addbackE[ABHitMapping[noABHitInd]]/keVPerBin);
                    tGamma1 = addbackT[ABHitMapping[noABHitInd]];
                }else{
                    //hit was a single non-addback hit
                    eGamma1 = (sortedEvt.noABHit[noABHitInd].energy/keVPerBin);
                    ...
                    if((!(usedHits & (1UL << noABHitInd2))) && ((!(abHitBuildFlags & (1UL << noABHitInd2))) || (!(usedABHits2 & (1UL << ABHitMapping[noABHitInd2]))))){
                        if(hitMap180deg[sortedEvt.noABHit[noABHitInd].core & 63U][sortedEvt.noABHit[noABHitInd2].core & 63U] != 0){
                            ...
                            if(abHitBuildFlags & (1UL << noABHitInd2)){
                                ...
                                eGamma2 = (addbackE[ABHitMapping[noABHitInd2]]/keVPerBin);
...
int main(int argc, char **argv){
    ...
    uint8_t discardAnySuppressor = 0;
    printf("Starting EGamma_modAB_mca_SMOL\n");

    if((argc != 6)&&(argc != 7)&&(argc != 8)&&(argc != 9)){
        cout << "Generates dmca singles spectra." << endl;
        cout << "Arguments: EGamma_modAB_mca_SMOL smol_file ABRadius same_clover_only forward_pos output_dmca_file keV_per_bin discard_pileup discard_on_any_suppression" << endl;
        ...
        cout << "  If *discard_on_any_suppression* is 1, then all SMOL events containing any fired suppressors will be dropped, regardless of suppressor position." << endl;
        return 0;
    }else{
        sfile = argv[1];
        ABRad = atof(argv[2]);
        ...
        if(argc > 6){
            ...
            if(argc > 7){
                ...
                if(argc > 8){
                    discardAnySuppressor = (uint8_t)(atoi(argv[8]));
                }
            }
        }
    }
    ...
    if(discardAnySuppressor == 1){
        printf("Discarding events where ANY suppressor fired.\n");
    }

    uint64_t numSepEvts = 0U;
    if(strcmp(dot + 1, "smol") == 0){
        printf("SMOL tree: %s\nAddback radius: %0.2f mm\nDetector position: %s\nClover addback only: %s\nOutput file: %s\n%0.2f keV per bin\n", sfile, ABRad, (forwardPos == 1) ? "forward" : "back", (sameCloverOnly == 1) ? "yes" : "no", outfile, keVPerBin);
        numSepEvts += SortData(sfile, ABRad, sameCloverOnly, forwardPos, keVPerBin, discardPileup, discardAnySuppressor);
        ...
        if((listfile=fopen(sfile,"r"))==NULL){
            ...
            while(!(feof(listfile))){//go until the end of file is reached
                if(fgets(str,256,listfile)!=NULL){ //get an entire line
                    ...
                    numSepEvts += SortData(str, ABRad, sameCloverOnly, forwardPos, keVPerBin, discardPileup, discardAnySuppressor);
                }
            }
        }
    }else{
