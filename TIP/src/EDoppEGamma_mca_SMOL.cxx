...
#define EDoppEGamma_mca_SMOL_cxx
#include "common.h"
#include "EDoppEGamma_mca_SMOL.h"

using namespace std;

void EDoppEGamma_mca_SMOL::WriteData(const char* outName){
...
void EDoppEGamma_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
...
int main(int argc, char **argv){
  ...
  EDoppEGamma_mca_SMOL *mysort = new EDoppEGamma_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  double eLow, eHigh;
  printf("Starting EDoppEGamma_mca_SMOL\n");

  if((argc != 5)&&(argc != 6)){
    cout << "Generates TIGRESS mca spectra for PID and time separated data." << endl;
    cout << "Arguments: EDoppEGamma_mca_SMOL smol_file EDoppGateLow EDoppGateHigh output_file keV_per_bin" << endl;
