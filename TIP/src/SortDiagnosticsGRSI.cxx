  ...
  for(Long64_t jentry = 0; jentry < analentries; jentry++){
    ...
    if(!tip && tigress){
      if(tigress->GetMultiplicity()>=0){
        tiptig_mult->Fill(0.,tigress->GetMultiplicity());
      }
      if(tigress->GetAddbackMultiplicity()>=0){
        tiptig_Amult->Fill(0.,tigress->GetAddbackMultiplicity());
      }
    }else if(tip && !tigress){
      if(tip->GetMultiplicity()>=0){
      ...
      if((tip->GetMultiplicity()>=0)||(tigress->GetAddbackMultiplicity()>=0)){
        tiptig_Amult->Fill(tip->GetMultiplicity(),tigress->GetAddbackMultiplicity());
      }
