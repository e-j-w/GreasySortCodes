...
#define EEGamma_AB_mca_SMOL_lastevents_cxx
#include "common.cxx"
#include "EEGamma_AB_mca_SMOL_lastevents.h"
...
void SortData(const char *sfile,  
  ...
  for(Long64_t jentry = 0; jentry < sentries; jentry+=increment){
    ...
    for(int ABpos = 0; ABpos < NGRIFPOS; ABpos++){
      ...
      int singlesE = (int)(addbackE[ABpos]/keVPerBin);
      ...
      for(int ABpos2 = ABpos+1; ABpos2 < NGRIFPOS; ABpos2++){
        ...
        //printf("Checking 180 degree condition for positions [%i %i]: %u.\n",ABpos*4,ABpos2*4,hitMap180degAB[ABpos*4][ABpos2*4]);
        if(hitMap180degAB[ABpos][ABpos2] != 0){
          Double_t tDiffSum = fabs(addbackTS[ABpos] - addbackTS[ABpos2]);
          //printf("tDiffSum: %f\n",tDiffSum);
          if((tDiffSum >= sumGateMin)&&(tDiffSum <= sumGateMax)){ //timing condition (sum)
            //printf("Passed timing condition.\n");

            int eGamma3 = (int)(addbackE[ABpos2]/keVPerBin);
            int eGammaSum = (int)((offset + (addbackE[ABpos]+addbackE[ABpos2])*gain + ((addbackE[ABpos]+addbackE[ABpos2])*(addbackE[ABpos]+addbackE[ABpos2]))*quad)/keVPerBin);
    ...
    for(int ABpos = 0; ABpos < NGRIFPOS; ABpos++){
      ...
      for(uint8_t i=0; i<numEGates; i++){
        if((addbackE[ABpos] >= gateELow[i])&&(addbackE[ABpos] <= gateEHigh[i])){
          for(int ABpos2 = 0; ABpos2 < NGRIFPOS; ABpos2++){
            ...
            if(ABpos2 != ABpos){
              ...
              {
                const Double_t tDiff = fabs(addbackT[ABpos2] - addbackT[ABpos]);
                uint8_t numCFDFail = 0;
                if(addbackNumCFDFail[ABpos]){
                  numCFDFail++;
                }
                if(addbackNumCFDFail[ABpos2]){
                  numCFDFail++;
                }
                if(evalCoincGateCFD(numCFDFail,tDiff)){
                  const int eGamma2 = (int)(addbackE[ABpos2]/keVPerBin);
              ...
              {
                const Double_t tDiffTS = fabs(addbackTS[ABpos2] - addbackTS[ABpos]);
              ...
              for(int ABpos3 = ABpos2+1; ABpos3 < NGRIFPOS; ABpos3++){
                ...
                if(hitMap180degAB[ABpos3][ABpos2] != 0){
                  ...
                  int firstSumHitInd = ABpos2;
                  int secondSumHitInd = ABpos3;
                  if(addbackTS[ABpos3] < addbackTS[ABpos2]){
                    firstSumHitInd = ABpos3;
                    secondSumHitInd = ABpos2;
                  }
                  int firstSumHitIndCFD = ABpos2;
                  int secondSumHitIndCFD = ABpos3;
                  if(addbackT[ABpos3] < addbackT[ABpos2]){
                    firstSumHitIndCFD = ABpos3;
                    secondSumHitIndCFD = ABpos2;
                  }

                  //Now check the sum timing condition
                  Double_t tDiffSum = addbackTS[secondSumHitInd] - addbackTS[firstSumHitInd];
                  //printf("tDiffSum: %f\n",tDiffSum);
                  if((tDiffSum >= sumGateMin)&&(tDiffSum <= sumGateMax)){ //timing condition (sum)

                    const int eSumGamma1 = (int)(addbackE[ABpos2]/keVPerBin);
                    const int eSumGamma2 = (int)(addbackE[ABpos3]/keVPerBin);
                    const int eGammaSum = (int)((offset + (addbackE[ABpos2]+addbackE[ABpos3])*gain + ((addbackE[ABpos2]+addbackE[ABpos3])*(addbackE[ABpos2]+addbackE[ABpos3]))*quad)/keVPerBin);
...

                    {
                      ...
                      const Double_t tDiff = fabs(addbackT[firstSumHitInd] - addbackT[ABpos]);
                      uint8_t numCFDFail = 0;
                      if(addbackNumCFDFail[ABpos]){
                        numCFDFail++;
                      }
                      if(addbackNumCFDFail[firstSumHitInd]){
                        numCFDFail++;
                      }
                    ...
                    {
                      const Double_t tDiffTS = fabs(addbackTS[firstSumHitInd] - addbackTS[ABpos]);
                      if((tDiffTS >= leCoincGateMin)&&(tDiffTS <= leCoincGateMax)){
                        //time coincident summing
                  ...
                  Double_t tDiffSumCFD = addbackT[secondSumHitIndCFD] - addbackT[firstSumHitIndCFD];
                  uint8_t numCFDFailSum = 0;
                  if(addbackNumCFDFail[firstSumHitIndCFD]){
                    numCFDFailSum++;
                  }
                  if(addbackNumCFDFail[secondSumHitIndCFD]){
                    numCFDFailSum++;
                  }
                  if(evalSumCorrGateCFD(numCFDFailSum, tDiffSumCFD)){
                    ...
                    //should be evaluated using the time of the first sum hit only
                    {
                      const Double_t tDiff = fabs(addbackT[firstSumHitIndCFD] - addbackT[ABpos]);
                      uint8_t numCFDFail = 0;
                      if(addbackNumCFDFail[ABpos]){
                        numCFDFail++;
                      }
...
int main(int argc, char **argv){
  ...
  printf("Starting EEGamma_AB_mca_SMOL_lastevents\n");

  if(argc < 6){
    cout << "Generates GRIFIFN gated spectra." << endl;
    cout << "Arguments: EEGamma_AB_mca_SMOL_lastevents smol_file_list num_gates EGateLow1 EGateHigh1 (EGateLow2 EGateHigh2...) num_sorts percent_of_events_1 (percent_of_events_2 ...) output_dmca_file_prefix keV_per_bin discard_pileup offset gain quad" << endl;
        ...
        if(i!=j){
          for(uint8_t i_cry=0; i_cry<4; i_cry++){
            for(uint8_t j_cry=0; j_cry<4; j_cry++){
              ...
              if(getGRIFFINVector((i*4)+i_cry,1).Angle(getGRIFFINVector((j*4)+j_cry,1))*180.0/PI > 170.0){ //same effect for any value down to 165 degrees
