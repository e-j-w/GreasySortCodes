...
#define EEDopp_noAB_mca_SMOL_cxx
#include "common.h"
#include "EEDopp_noAB_mca_SMOL.h"

using namespace std;

void EEDopp_noAB_mca_SMOL::WriteData(const char* outName){
...
void EEDopp_noAB_mca_SMOL::SortData(char const *sfile, const double eLow, const double eHigh, const double keVPerBin){
...
int main(int argc, char **argv){
  ...
  EEDopp_noAB_mca_SMOL *mysort = new EEDopp_noAB_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  double eLow, eHigh;
  printf("Starting EEDopp_noAB_mca_SMOL\n");
  ...
  if((argc != 5)&&(argc != 6)){
    ...
    cout << "Arguments: EEDopp_noAB_mca_SMOL smol_file EGateLow EGateHigh output_file keV_per_bin" << endl;
