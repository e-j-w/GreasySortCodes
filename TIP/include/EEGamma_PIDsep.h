#ifndef EEGamma_PIDsep_h
#define EEGamma_PIDsep_h
...
void EEGamma_PIDsep::Initialise(){
  ...
  for(int i=0; i<MAX_NUM_PARTICLE+1; i++){
    for(int j=0; j<MAX_NUM_PARTICLE+1; j++){
      if((i+j)<=MAX_NUM_PARTICLE){
        //TIGRESS
        tigEE_xayp[i][j] = new TH2F(Form("TIGRESS addback energy vs. addback energy (%ip%ia gate)",i,j),Form("TIGRESS addback energy vs. addback energy (%ip%ia gate)",i,j),8192,0,8192,8192,0,8192);
