...
void SeparatorTiming::VerifyTimingSepEvt(TTigress *tigress, TTigress *tigressSep, TTip *tip, TTip *tipSep){

  if(tigress && tip){
    if(tip->GetMultiplicity()>MAXNUMTIPHIT){
      cout << "WARNING: event has too many TIP hits (" << tip->GetMultiplicity() << ")!" << endl;
      return;
    }
    if(tigress->GetAddbackMultiplicity()>MAXNUMTIGHIT){
      cout << "WARNING: event has too many TIGRESS hits (" << tigress->GetAddbackMultiplicity() << ")!" << endl;
      return;
    }
    ...
    if(tigressSep && tipSep){
      ...
      if(true){
        cout << "Original event and separated event do not match!" << endl;
        for(int i = 0; i < tip->GetMultiplicity() ; i++){
        ...
        cout << "Event passed: " << tip->GetMultiplicity() << " " << tigress->GetMultiplicity() << endl;
      }
    }else{
      cout << "WARNING: No TIGRESS or TIP data in separated entry!" << endl;
    }
  }else{
    cout << "WARNING: No TIGRESS or TIP data in original entry!" << endl;
  }
...
void SeparatorTiming::SortData(char const *afile, char const *calfile, char const *outfile){
  ...
  if(AnalysisTree->FindBranch("TTigress.TDetector")){
    AnalysisTree->SetBranchAddress("TDetector", &tigressDet);
  }else{
    cout << "ERROR: Branch 'TTigress.TDetector' not found!" << endl;
    return;
  }
  ...
  TTigress *tigressSep = 0;
  TTip *tipSep = 0;
  outTree->Branch("TTigress","TTigress",&tigressSep); //defining/filling only tigress doesn't help
  outTree->Branch("TTip","TTip",&tipSep);
    ...
    if(tigress && tip){
      ...
      if(tip->GetMultiplicity()>MAXNUMTIPHIT){
        ...
        if(passedtimeGate&(1ULL<<TIPTIGFLAG)){
          ...
          //VerifyTimingSepEvt(tigress,tigressSep,tip,tipSep); //copying fTigressBits in TTigress::Copy causes segments to no longer be incorrectly copied, but energies are still wrong
