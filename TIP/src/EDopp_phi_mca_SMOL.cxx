...
#define EDopp_phi_mca_SMOL_cxx
#include "common.h"
#include "EDopp_phi_mca_SMOL.h"

using namespace std;

void EDopp_phi_mca_SMOL::WriteData(const char* outName){
...
void EDopp_phi_mca_SMOL::SortData(char const *sfile, double keVPerBin){
...
int main(int argc, char **argv){
  ...
  EDopp_phi_mca_SMOL *mysort = new EDopp_phi_mca_SMOL();

  const char *sfile;
  const char *outfile;
  double keVPerBin = 1.0;
  printf("Starting EDopp_phi_mca_SMOL\n");
  ...
  if((argc != 3)&&(argc != 4)){
    cout << "Generates TIGRESS mca spectra for PID and time separated data, separated by phi bin." << endl;
    cout << "Arguments: EDopp_phi_mca_SMOL smol_file output_file keV_per_bin" << endl;
