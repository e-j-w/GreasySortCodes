...
void InitialiseHists() {
    ...
    //decay curve Spectra
    counts_time = new TH1F("Counts_vs_time", "Addback Energy vs. time", 32768, 0, 32768);
    counts_time->GetXaxis()->SetTitle("Time (min)");
    counts_time->GetYaxis()->SetTitle("Counts / minute");
    dcList->Add(counts_time);
    counts_time_685 = new TH1F("Counts_vs_time_685", "Addback Energy vs. time (685 keV gate)", 32768, 0, 32768);
    counts_time_685->GetXaxis()->SetTitle("Time (min)");
    counts_time_685->GetYaxis()->SetTitle("Counts / minute");
    dcList->Add(counts_time_685);
    counts_time_934 = new TH1F("Counts_vs_time_934", "Addback Energy vs. time (934 keV gate)", 32768, 0, 32768);
    counts_time_934->GetXaxis()->SetTitle("Time (min)");
    counts_time_934->GetYaxis()->SetTitle("Counts / minute");
    dcList->Add(counts_time_934);
    counts_time_1477 = new TH1F("Counts_vs_time_1477", "Addback Energy vs. time (1477 keV gate)", 32768, 0, 32768);
    counts_time_1477->GetXaxis()->SetTitle("Time (min)");
    counts_time_1477->GetYaxis()->SetTitle("Counts / minute");
    dcList->Add(counts_time_1477);
    addE_time = new TH2F("Addback_Energy_vs_time", "Addback Energy vs. time", 2048, 0, 4096, 32768, 0, 32768);
    addE_time->GetXaxis()->SetTitle("Energy (keV)");
    addE_time->GetYaxis()->SetTitle("Time (min)");
    dcList->Add(addE_time);
    hpgeE_time = new TH2F("NonAddback_Energy_vs_time", "Single-crystal Energy vs. time", 2048, 0, 4096, 32768, 0, 32768);
