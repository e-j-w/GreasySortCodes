...
// Search for the peaks in the charge spectrum to initialize gain and offset
 ...
 std::cout << "\nFound n peaks " << nfound; fflush(stdout);


 Double_t * xpeaks = s->GetPositionX();
//this part here should skip any channels that have all the peaks below some arbitrary (600) threshold. This prevents seg-faults in the next loop over nfound>3
 std::vector<double>myXPeaks;
 for (int i = 0; i < nfound; i++){
	myXPeaks.push_back(xpeaks[i]);
 }
 if (std::all_of(myXPeaks.begin(),myXPeaks.end(),[](const Double_t &d){return d < 600;})){
	return F;
 }

 if (nfound>=3){  
    for (int oo=0; oo<=nfound; oo++){
       if (xpeaks[oo]>700 and xpeaks[oo]<10000){
...
// For analyzing SHARC data, (ie. Channel##) with oldsharc.root or newsharc.root
     ...
     // cout << "\nEnter the histogram file name.\n";
      ...
      cout << iii << " :";   
      if (NULL == h) continue; //this skips any channel that the script didn't create a histogram for. Probably a disabled channel or no events on that channel
      //if (947 == iii || 962 == iii || 1040 == iii || 1120 == iii || 1159 == iii || 1167 == iii || 1229 == iii) continue; //these are channels in SHARC with lots of noise, so it seg-faults when it tries to auto-fit them.
      TF1* f=tasf(h); 
...
//opening a calibration file to write them directly to it
   TFile *tf = new TFile("../FragmentTrees/fragment55466_000.root");
   TChannel *tchan = NULL;
   TTree *tree = (TTree*)tf->Get("FragmentTree");
   TFragment *frag = 0;
   tree->SetBranchAddress("TFragment",&frag);
   TChannel::ReadCalFromTree(tree);
   for (int j = 0; j < workingchannels.size(); j++){
	std::cout << "\nworkingchannel is " << workingchannels[j] << " and eng coeffs are a0 = " << OFFSET[j] << " a1= " << GAIN[j];
	tchan = TChannel::GetChannelByNumber((int)workingchannels[j]);
	std::cout << "\nGot the TChannel";fflush(stdout);
	tchan->DestroyENGCal();
	//std::cout << "\nDestroyed eng cal";fflush(stdout);
	std::cout << "\ntrying to add te eng cal stuff now"; fflush(stdout);
	tchan->AddENGCoefficient(OFFSET[j]);
	tchan->AddENGCoefficient(GAIN[j]);
	std::cout << "\nTrying to write to cal file now"; fflush(stdout);
	TChannel::WriteCalFile("autoCalOutput.cal");
   }
   tf->Close();
