...
// Change below with actual target thicknesses
  ...
  TReaction *sr93_rxn = new TReaction("kr86", "d", "p", "sr94", EBeam, 0, true);
 ...
 */
  ...
  //TFile* cutFile = new TFile("test_cut.root");
  ...
  for (int jentry = 0; jentry < analentries; jentry++)
  {
	...
	for (int i = 0; i < sharc->GetMultiplicity(); i++){
	   ...
	   if (0 != sharc_hit->GetPad().GetDetector()){
		sharc_dE_E->Fill(sharc_hit->GetPad().GetEnergy(),eSharc);
	   } 
	...
	//	}
			...
			if(cut->IsInside(TMath::RadToDeg()*posSharc.Theta(),eSharc)) {		    
