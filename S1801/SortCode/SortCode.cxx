...
void SortCode::SortData(char const *afile, char const *calfile, char const *outfile, char const *target = "NULL")
{
  ...
  TSRIM *srim_21neD = new TSRIM;
  srim_21neD->ReadEnergyLossFile("ne21_in_cd2.txt");
  TSRIM *srim_21naD = new TSRIM;
  srim_21naD->ReadEnergyLossFile("na21_in_cd2.txt");
  //double EBeam = 4.0 * 20.99384669; //21Ne beam
  double EBeam = 4.0 * 20.99765470; //21Na beam
  printf("Beam energy: %f MeV\n", EBeam);

// Change below with actual target thicknesses
  if (strcmp(target, "three") == 0){
    printf("Using target %s - 2.151 \u03BCm CD2 \n", target); // Assume reaction happens in middle
    EBeam = srim_21neD->GetAdjustedEnergy(EBeam * 1000, 2.151/2., 0.001) / 1000;               // GetAdjustedEnergy(energy, target thickness um,step size)
    //EBeam = srim_21naD->GetAdjustedEnergy(EBeam * 1000, 0.19, 0.001) / 1000;
  }else if (strcmp(target, "two") == 0){
    printf("Using target %s - 2.656 \u03BCm CD2  \n", target);
    EBeam = srim_21neD->GetAdjustedEnergy(EBeam * 1000, 2.656/2., 0.001) / 1000; // GetAdjustedEnergy(energy, target thickness um,step size)
    //EBeam = srim_21naD->GetAdjustedEnergy(EBeam * 1000, 0.485, 0.001) / 1000;
  }else if (strcmp(target, "one") == 0){
    printf("Using target %s - 5.989 \u03BCm CD2  \n", target);
    EBeam = srim_21neD->GetAdjustedEnergy(EBeam * 1000, 5.989/2., 0.001) / 1000; // GetAdjustedEnergy(energy, target thickness um,step size)
    //EBeam = srim_21naD->GetAdjustedEnergy(EBeam * 1000, 0.485, 0.001) / 1000;
  }else{
  ...
  //TReaction *ne21 = new TReaction("ne21", "d", "p", "ne22", EBeam, 0, true);
  TReaction *ne21 = new TReaction("na21", "d", "p", "na22", EBeam, 0, true);
  ...
  double_t betaDoppler = 0.0859; //beta for Doppler correction
        ...
        pgac->Fill(em_hit->GetPosition().X(), em_hit->GetPosition().Y());
   	       ...
   	       si_hit = emma->GetSiHit(k);
						...
						if(ne22_cut->IsInside(tempIC + si_hit->GetEnergy(),tempIC))
							pgac_ne22->Fill(em_hit->GetPosition().X(), em_hit->GetPosition().Y());
						if(na22_cut->IsInside(tempIC + si_hit->GetEnergy(),tempIC))
							pgac_na22->Fill(em_hit->GetPosition().X(), em_hit->GetPosition().Y());
...
//						std::cout	<< "BOO " << tigress->GetMultiplicity()
	//										<< std::endl;
